...
export function drawLegend() {
    ...
    legend.className = "fhc-legend-container fhc";
...
function _buildLegendHTML() {
    let html = `<h3 class="fhc-legend-header">${game.i18n.localize("FILRODENSHEX.Legend.Title")}</h3>`;
    html += `<div class="fhc-legend-biomes">`;
    ...
    for (const [key, config] of Object.entries(FILRODENSHEX.BIOMES)) {
        ...
        html += `
        <div class="fhc-legend-row">
            <div class="fhc-legend-swatch" style="background-color: ${hexColor};"></div>
            <span>${label}</span>
        </div>`;
    }
